...
from apps.api.serializers import BookSerializer, PersonSerializer, CitySerializer
...
class PersonView(APIView):
    ...
    def get(self, request, pk=None):
        if not pk:
            return Response({
                'data': [PersonSerializer(instance=obj).data for obj in Person.objects.all()],
                'success': True
            }, status=HTTPStatus.OK)
        try:
        ...
        return Response({
            'data': PersonSerializer(instance=obj).data,
            'success': True
        }, status=HTTPStatus.OK)
    ...
    def put(self, request, pk):
        ...
        serializer = PersonSerializer(instance=obj, data=request.POST, partial=True)
        if not serializer.is_valid():
            return Response(data={
                **serializer.errors,
                'success': False
